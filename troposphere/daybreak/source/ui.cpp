...
namespace dbk {
            ...
            switch (activated_button->id) {
                case InstallButtonId:
                {
                    ...
                    if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereHasRcmBugPatch), &has_rcm_bug_patch))) {
                        ChangeMenu(std::make_shared<ErrorMenu>("Ein Fehler ist aufgetreten", "RCM Status konnte nicht ermittelt werden.", rc));
                        return;
                    }

                    if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereEmummcType), &is_emummc))) {
                        ChangeMenu(std::make_shared<ErrorMenu>("Ein Fehler ist aufgetreten", "emuMMC Status konnte nicht ermittelt werden.", rc));
                        return;
                    }
                    ...
                    if (is_erista && has_rcm_bug_patch && !is_emummc) {
                        ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, file_menu, "Warnung: Gepatchte Konsole", "fuses werden verbraucht oder Switch wird unbrauchbar."));
                    } else {
    ...
    void FileMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Waehle Update-Ordnerpfad", x, y, WindowWidth, WindowHeight);
    ...
    ValidateUpdateMenu::ValidateUpdateMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu), m_has_drawn(false), m_has_info(false), m_has_validated(false) {
        ...
        if (R_FAILED(this->GetUpdateInformation())) {
            ...
            this->LogText("Ueberpruefe Update, dauert einen Moment...\n");
        }
    }

    Result ValidateUpdateMenu::GetUpdateInformation() {
        Result rc = 0;
        this->LogText("Update-Ordnerpfad %s\n", g_update_path);

        /* Attempt to get the update information. */
        if (R_FAILED(rc = amssuGetUpdateInformation(&m_update_info, g_update_path))) {
            if (rc == 0x1a405) {
                this->LogText("Kein Update im Ordner.\nStelle sicher das ncas richtig benannt sind!\nErgebnis: 0x%08x\n", rc);
            } else {
                this->LogText("Keine Updateinformationen gefunden.\nErgebnis: 0x%08x\n", rc);
            }
            return rc;
        }
    ...
    void ValidateUpdateMenu::ValidateUpdate() {
        ...
        if (R_SUCCEEDED(m_validation_info.result)) {
            ...
            const u32 version = m_validation_info.invalid_key.version;
            this->LogText("Ueberpruefung fehlgeschlagen mit Ergebnis: 0x%08x\n", m_validation_info.result);
            this->LogText("Fehlender Inhalt:\n- Programm ID: %016lx\n- Version: %d.%d.%d\n", m_validation_info.invalid_key.id, (version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf);
            ...
            switch (activated_button->id) {
                ...
                case ContinueButtonId:
                    ...
                    if (m_update_info.exfat_supported && R_FAILED(m_validation_info.exfat_result)) {
                        next_menu = std::make_shared<WarningMenu>(g_current_menu, next_menu, "Warnung: exFAT firmware is missing or corrupt", "Are you sure you want to proceed?");
                    }
                    ...
                    if (EncodeVersion((version >> 26) & 0x1f, (version >> 20) & 0x1f, (version >> 16) & 0xf) > g_supported_version) {
                        next_menu = std::make_shared<WarningMenu>(g_current_menu, next_menu, "Warnung: firmware is too new and not known to be supported", "Are you sure you want to proceed?");
                    }
    ...
    void ValidateUpdateMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Updateinformationen", x, y, WindowWidth, WindowHeight);
    ...
    ChooseResetMenu::ChooseResetMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu) {
        ...
        this->AddButton(PreserveSettingsButtonId, "Daten behalten", x + HorizontalInset + button_width + ButtonHorizontalGap, y + TitleGap, button_width, ButtonHeight);
    ...
    void ChooseResetMenu::Update(u64 ns) {
        ...
        if (const Button *activated_button = this->GetActivatedButton(); activated_button != nullptr) {
            ...
            if (g_exfat_supported) {
                ...
                next_menu = std::make_shared<WarningMenu>(g_current_menu, std::make_shared<InstallUpdateMenu>(g_current_menu), "Bereit fuer das Update", "Sicher das du fortfahren willst?");
            }

            if (g_reset_to_factory) {
                ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, next_menu, "Warnung: Werkseinstellung gewaehlt", "Installierte Spiele und Daten werden permanent geloescht."));
            } else {
                ChangeMenu(next_menu);
            }
        }
    ...
    void ChooseResetMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Waehle Einstellungsmodus", x, y, WindowWidth, WindowHeight);
        this->DrawButtons(vg, ns);
    }
    ...
    ChooseExfatMenu::ChooseExfatMenu(std::shared_ptr<Menu> prev_menu) : Menu(prev_menu) {
        ...
        /* Add buttons. */
        this->AddButton(Fat32ButtonId, "Installieren (FAT32)", x + HorizontalInset, y + TitleGap, button_width, ButtonHeight);
        this->AddButton(ExFatButtonId, "Installieren (FAT32 + exFAT)", x + HorizontalInset + button_width + ButtonHorizontalGap, y + TitleGap, button_width, ButtonHeight);
    ...
    void ChooseExfatMenu::Update(u64 ns) {
        ...
        if (const Button *activated_button = this->GetActivatedButton(); activated_button != nullptr) {
            ...
            ChangeMenu(std::make_shared<WarningMenu>(g_current_menu, std::make_shared<InstallUpdateMenu>(g_current_menu), "Bereit fuer Einrichten des Updates", "Sicher das du fortfahren willst?"));
        }
    ...
    void ChooseExfatMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Waehle Treibervariante", x, y, WindowWidth, WindowHeight);
        this->DrawButtons(vg, ns);
    }
    ...
    Result InstallUpdateMenu::TransitionUpdateState() {
        ...
        if (m_install_state == InstallState::NeedsSetup) {
            ...
            if (R_FAILED(rc = amssuSetupUpdate(nullptr, UpdateTaskBufferSize, g_update_path, g_use_exfat))) {
                this->LogText("Fehler beim Einrichten des Updates.\nErgebnis: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }

            /* Log setup completion. */
            this->LogText("Update komplett eingerichtet.\n");
            ...
            if (R_FAILED(rc = amssuRequestPrepareUpdate(&m_prepare_result))) {
                this->LogText("Fehler beim Anfordern der Update-Vorbereitung.\nErgebnis: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }
            ...
            if (R_FAILED(rc = asyncResultWait(&m_prepare_result, 0)) && rc != 0xea01) {
                this->LogText("Fehler beim Ueberpruefen der Update-Vorbereitung.\nErgebnis: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            } else if (R_SUCCEEDED(rc)) {
                if (R_FAILED(rc = asyncResultGet(&m_prepare_result))) {
                    this->LogText("Fehler beim Vorbereiten des Updates.\nErgebnis: 0x%08x\n", rc);
                    this->MarkForReboot();
                    return rc;
                }
            }
            ...
            if (R_FAILED(rc = amssuHasPreparedUpdate(&prepared))) {
                this->LogText("Fehler beim Ueberpruefen, ob das Update vorbereitet wurde.\nErgebnis: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }
            ...
            if (R_FAILED(rc = amssuGetPrepareUpdateProgress(&update_progress))) {
                this->LogText("Fehler beim Ueberpruefen des Update-Fortschritts.\nErgebnis: 0x%08x\n", rc);
                this->MarkForReboot();
                return rc;
            }
            ...
            if (R_FAILED(rc = amssuApplyPreparedUpdate())) {
                this->LogText("Fehler beim installieren des Updates.\nErgebnis: 0x%08x\n", rc);
            } else {
                /* Log success. */
                this->LogText("Update erfolgreich installiert.\n");
                    ...
                    if (R_FAILED(rc = nsResetToFactorySettingsForRefurbishment())) {
                        ...
                        if (rc == MAKERESULT(Module_Libnx, LibnxError_IncompatSysVer)) {
                            if (R_FAILED(rc = nsResetToFactorySettings())) {
                                this->LogText("Fehler beim Zuruecksetzen auf Werkseinstellungen.\nErgebnis: 0x%08x\n", rc);
                                this->MarkForReboot();
                                return rc;
                            }
                        } else {
                            this->LogText("Fehler beim Zuruecksetzen auf die Werkseinstellungen für die Wiederaufbereitung.\nErgebnis: 0x%08x\n", rc);
                            this->MarkForReboot();
                            return rc;
                        }
                    }
    ...
    void InstallUpdateMenu::Draw(NVGcontext *vg, u64 ns) {
        ...
        DrawWindow(vg, "Installiere Update", x, y, WindowWidth, WindowHeight);
    ...
    bool InitializeMenu(u32 screen_width, u32 screen_height) {
        ...
        if (R_FAILED(rc = splGetConfig(static_cast<SplConfigItem>(ExosphereApiVersionConfigItem), &version))) {
            ChangeMenu(std::make_shared<ErrorMenu>("Atmosphere nicht gefunden", "Daybreak benoetigt atmosphere.", rc));
            return false;
        }
        ...
        if (!ams_supports_sysupdate_api) {
            ChangeMenu(std::make_shared<ErrorMenu>("Veraltete atmosphere Version", "Daybreak benoetigt Atmosphere 0.14.0 oder spaeter.", rc));
            return false;
        }
